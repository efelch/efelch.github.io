...
const gameState = {
    ...
    riverHintStep: 0,
    // Cave hidden door state
    caveFootprintsSeen: false,
    caveLeverRevealed: false,
    caveHiddenDoorOpen: false,
    // Cave global illumination state (once lit, cave is safe from grues)
    caveTorchesLit: false,
    // Temple torch puzzle state (3x3 grid of booleans)
    templeSarcophagusOpened: false,
    templeTorches: [
        [false, false, false],
        [false, false, false],
        [false, false, false]
    ],
    templeStaircaseRevealed: false
};

// Initialize the temple torch puzzle with a random, solvable configuration.
// Delegates to the pure templeTorches module so rules live in one place.
function randomizeTempleTorches() {
    gameState.templeStaircaseRevealed = false;
    try {
        if (window.templeTorches && typeof window.templeTorches.randomize === 'function') {
            gameState.templeTorches = window.templeTorches.randomize();
        } else {
            // Fallback: preserve existing state shape if module missing
...
// Expose randomizer for other modules (restart/new game)
try { window.randomizeTempleTorches = randomizeTempleTorches; } catch (_) {}
...
function move(direction) {
    ...
    if (nextRoom) {
        ...
        if (rooms[nextRoom]) {
            ...
            // Grue danger zones: entering a dark room without a flashlight is fatal.
            // Some rooms might be permanently lit later (e.g. caveTorchesLit).
            const targetRoom = rooms[gameState.currentRoom];
            if (targetRoom.isDark && !gameState.flashlightOn && !gameState.caveTorchesLit) {
                gameState.isGameOver = true;
                updateStatusBar();
                return buildRoomDescription(gameState.currentRoom);
            }
            updateStatusBar();
...
function updateStatusBar() {
    ...
    // Announce score increase for screen readers
    if (typeof gameState.lastAnnouncedScore === 'undefined') {
        gameState.lastAnnouncedScore = gameState.score;
    } else if (gameState.score > gameState.lastAnnouncedScore) {
        const diff = gameState.score - gameState.lastAnnouncedScore;
        announce(`Score increased by ${diff}. Total score: ${gameState.score}`);
        gameState.lastAnnouncedScore = gameState.score;
    }
...
function calculateMaxScore() {
    ...
    totalScore += 10; // temple torch puzzle solved (staircase revealed)
    totalScore += 5;  // trophyInCase
    totalScore += 1;  // take album
    totalScore += 1;  // take trophy
    totalScore += 1;  // take flashlight
    totalScore += 1;  // take lighter
    totalScore += 2;  // take both waystone shards (green + red)
...
// --- Room description builder with dynamic item presence lines ---
function buildRoomDescription(roomId) {
    const rId = roomId || gameState.currentRoom;
    const room = rooms[rId];
    if (!room) return '';

    // Resolve base description (supports string or function)
    const baseDescProp = room.description;
    let base = (typeof baseDescProp === 'function') ? baseDescProp() : (baseDescProp || '');

    const items = Array.isArray(room.items) ? room.items : [];
    const lines = [];

    // Small helper for a/an articles
    const article = (name) => (/^[aeiou]/i.test(name.trim()) ? 'An' : 'A');

    for (const id of items) {
        // Skip if already taken
        if (gameState.inventory && gameState.inventory.includes(id)) continue;
        const meta = itemData[id] || {};
        // Respect item visibility gates
        if (typeof meta.isVisible === 'function' && !meta.isVisible(gameState)) continue;

        // Compute presence line
        let line = '';
        const roomLine = meta.roomLine;
        if (typeof roomLine === 'function') {
            line = roomLine(gameState, rId) || '';
        } else if (typeof roomLine === 'string') {
            line = roomLine;
        } else {
            // Fallback generic sentence
            const n = String(id);
            line = `${article(n)} ${n} ${n.includes(' ') ? 'are' : 'is'} here.`; // simple grammar; can be overridden via roomLine
        }
